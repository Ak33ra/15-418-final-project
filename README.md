...
To run the Llama experiments, an authorized Hugging Face token is needed, with approval from Meta. You can request access via Hugging Face here: https://huggingface.co/meta-llama/Llama-3.2-3B-Instruct. Additionally, generate a Hugging Face token and place the following in your `.bashrc`:

```bash
export HF_TOKEN="hf_XXXXXXXX"                  # your token
